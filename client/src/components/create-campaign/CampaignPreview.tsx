    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-4">
          <section>
            <h4 className="text-xs font-bold text-primary uppercase tracking-wider mb-2">Campaign Details</h4>
          ...
          <section>
            <h4 className="text-xs font-bold text-primary uppercase tracking-wider mb-2">Requirements</h4>
        ...
        <div className="space-y-4">
          <section>
            <h4 className="text-xs font-bold text-primary uppercase tracking-wider mb-2">Action Summary</h4>
            <div className="p-3 bg-primary/5 rounded-lg border border-primary/10 space-y-2 max-h-[150px] overflow-y-auto">
              {isHolder ? (
                ...
                values.actions?.map((action: any, i: number) => (
                  <div key={i} className="flex items-center gap-2 text-xs py-1 border-b border-primary/5 last:border-0">
                    {action.type === 'twitter_follow' && <Twitter className="h-3 w-3" />}
                    {action.type === 'telegram_join' && <Send className="h-3 w-3" />}
                    {action.type === 'website' && <Globe className="h-3 w-3" />}
                    <span className="flex-1 truncate">{action.title}</span>
                    <span className="font-mono text-primary">{action.rewardAmount}</span>
                  </div>
                ))
              )}
            </div>
          ...
          <section>
            <h4 className="text-xs font-bold text-primary uppercase tracking-wider mb-2">Estimated Fees</h4>
            <div className="p-3 bg-primary/5 rounded-lg border border-primary/10 space-y-1">
