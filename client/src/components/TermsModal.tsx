  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] glass-card border-white flex flex-col p-0 overflow-hidden shadow-[0_0_60px_rgba(255,255,255,0.2)] ring-2 ring-white/50">
        <DialogHeader className="p-6 border-b border-white/5 bg-gradient-to-br from-primary/10 via-transparent to-transparent relative">
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1 text-left">
              <DialogTitle className="font-display font-black text-4xl uppercase tracking-tighter italic text-white">
                Legal Agreement
              </DialogTitle>
              <p className="text-sm text-muted-foreground mt-1 font-bold uppercase tracking-widest leading-relaxed max-w-[280px]">
                Please read and accept our Terms to continue.
              </p>
            </div>
            <div className="shrink-0 relative">
              <div className="absolute inset-0 bg-primary/10 blur-[30px] rounded-full scale-125" />
              <img 
                src={PLATFORM_CONFIG.ASSETS.LEGAL_BANNER} 
                alt="Legal mascot"
                className="w-48 h-48 object-contain relative z-10 drop-shadow-[0_0_30px_rgba(34,197,94,0.4)]"
              />
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-hidden p-0 relative bg-black/40">
          <ScrollArea 
            className="h-[420px] w-full px-10 py-6" 
            onScrollCapture={handleScroll}
          >
            <div className="space-y-8 text-[15px] text-muted-foreground/90 leading-relaxed pb-8">
              <section>
                <h3 className="text-white font-black text-xs uppercase tracking-[0.2em] mb-3">1. Acceptance of Terms</h3>
              ...
              <section>
                <h3 className="text-white font-black text-xs uppercase tracking-[0.2em] mb-3">2. Eligibility & Risk</h3>
              ...
              <section>
                <h3 className="text-white font-black text-xs uppercase tracking-[0.2em] mb-3">3. Prohibited Conduct</h3>
              ...
              <section>
                <h3 className="text-white font-black text-xs uppercase tracking-[0.2em] mb-3">4. Reward Verification</h3>
              ...
              <section>
                <h3 className="text-white font-black text-xs uppercase tracking-[0.2em] mb-3">5. Privacy & Data</h3>
              ...
              <section>
                <h3 className="text-white font-black text-xs uppercase tracking-[0.2em] mb-3">6. Limitation of Liability</h3>
              ...
              <div className="pt-4 border-t border-white/5 italic text-[11px] font-medium text-muted-foreground/40 text-center">
        ...
        <DialogFooter className="p-8 border-t border-white/5 bg-black/60 flex-col gap-4">
          <div 
            className={`flex items-start space-x-3 w-full p-4 rounded-xl transition-all duration-500 ${hasReadToBottom ? 'bg-primary/5 border border-primary/20 shadow-[0_0_20px_rgba(34,197,94,0.05)]' : 'bg-white/5 border border-white/5 opacity-50'}`}
          >
            <Checkbox 
              id="terms" 
              checked={hasAccepted} 
            ...
            <Label 
              htmlFor="terms" 
              className={`text-xs font-black uppercase tracking-widest leading-tight cursor-pointer transition-colors ${!hasReadToBottom ? 'text-muted-foreground' : 'text-white hover:text-primary'}`}
            >
              {!hasReadToBottom ? "Scroll to review complete terms" : "I have read and accept the terms of service"}
            </Label>
          </div>
          <Button 
            className="w-full bg-primary text-primary-foreground font-black uppercase tracking-widest h-14 text-md shadow-[0_0_30px_rgba(34,197,94,0.3)] hover:shadow-[0_0_40px_rgba(34,197,94,0.5)] transition-all disabled:opacity-30 rounded-xl border-none"
